...
# 方法二：直接利用pandas库的方法读取mysql的数据表
data2=pd.read_sql(sql, conn1)
print(data2.head())
print(data2.shape[0])
conn1.close()
...
# 设置API参数
offset=50
URL1='http://restapi.amap.com/v3/place/text?&key=&offset='+ str(offset) + '&types=991001&output=json&page='
URL2="&keywords="
URL3="&city="
...
def save_csv(projectid,  page):
    ...
    df.to_csv("data_output/"+str(projectid)+"_" +  str(page)+".txt", index=False, sep="\t")
...
def main():
    ...
    for index in range(project_number):
        keywords=str(data2.projectname[index])
        city=str(data2.cityname[index])

        url=URL1+'1'+URL2+keywords+URL3+city
        get_url(url)
        page=int(total_num)//offset + 2

        for i in range(1, page):
            url = URL1+str(i)+URL2+keywords+URL3+city
            get_url(url)
...
# mysql数据库里面创建xiangmu_door_info
# 语句
# create table xiangmu_door_info (
# id char(20) not null,
# name char(128),
# type char(128),
# typecode char(16),
